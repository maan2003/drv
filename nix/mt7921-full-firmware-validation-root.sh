...
@grep@ -F '"association_request_contract":"mt7921-supported-subset-v1"' @artifact_identity@ >/dev/null
@grep@ -F '"canonical_association_fixture_sha256":"aa0306b8149896b679356f23657c7a77b49b82bfd4d485e3009831e4473437c4"' @artifact_identity@ >/dev/null
@grep@ -F '"runtime_association_hash_policy":"input-dependent"' @artifact_identity@ >/dev/null
@grep@ -F '"association_capability_input_source":"firmware-nic-capability-to-softmac-query-band-v1"' @artifact_identity@ >/dev/null
@grep@ -F '"association_transformation_contract":"device-authoritative-ht-vht-v1"' @artifact_identity@ >/dev/null
