...
@grep@ -F '"passive_m1_diagnostic_contract":"linux-6.18.40-safe-read-rx-dma-v1"' @artifact_identity@ >/dev/null
@grep@ -F '"no_m1_result":"no_m1_at_rx_dma_ambiguous"' @artifact_identity@ >/dev/null
@grep@ -F '"consuming_mib_reads":false' @artifact_identity@ >/dev/null
@grep@ -F '"definitive_attribution_requires":"independent-ap-or-over-air-witness"' @artifact_identity@ >/dev/null
