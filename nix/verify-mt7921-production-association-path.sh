...
grep -F 'production_association_profile_from_query_and_regulatory(' "$adapter" >/dev/null
...
grep -F 'configuration.ht_cap = Some(fidl_ieee80211::HtCapabilities {' "$adapter" >/dev/null
grep -F 'configuration.vht_cap = Some(fidl_ieee80211::VhtCapabilities {' "$adapter" >/dev/null
grep -F 'successful association response reaches DeviceOps' "$adapter" >/dev/null
grep -F 'production_association_profile_from_query_and_regulatory(' "$binary" >/dev/null
...
grep -F 'canonical_sha256 != "5449fa5acf5317259694bb400a04d6ba8e169f99cf555583a424b3530f8a63c4"' "$binary" >/dev/null
