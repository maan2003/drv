...
@grep@ -Fx 'ASSOCIATION_REQUEST_CONTRACT=mt7921-supported-subset-v1' "$manifest" >/dev/null
@grep@ -Fx 'CANONICAL_ASSOCIATION_FIXTURE_SHA256=aa0306b8149896b679356f23657c7a77b49b82bfd4d485e3009831e4473437c4' "$manifest" >/dev/null
@grep@ -Fx 'RUNTIME_ASSOCIATION_HASH_POLICY=input-dependent' "$manifest" >/dev/null
@grep@ -Fx 'ASSOCIATION_CAPABILITY_INPUT_SOURCE=firmware-nic-capability-to-softmac-query-band-v1' "$manifest" >/dev/null
@grep@ -Fx 'ASSOCIATION_TRANSFORMATION_CONTRACT=device-authoritative-ht-vht-v1' "$manifest" >/dev/null
...
@grep@ -F '"association_request_contract":"mt7921-supported-subset-v1"' "$identity" >/dev/null
@grep@ -F '"canonical_association_fixture_sha256":"aa0306b8149896b679356f23657c7a77b49b82bfd4d485e3009831e4473437c4"' "$identity" >/dev/null
@grep@ -F '"runtime_association_hash_policy":"input-dependent"' "$identity" >/dev/null
@grep@ -F '"association_capability_input_source":"firmware-nic-capability-to-softmac-query-band-v1"' "$identity" >/dev/null
@grep@ -F '"association_transformation_contract":"device-authoritative-ht-vht-v1"' "$identity" >/dev/null
...
@grep@ -F '"safe_read_registers":"0xd4208,0xd4528,0xd452c"' "$identity" >/dev/null
@grep@ -F '"consuming_mib_reads":false' "$identity" >/dev/null
@grep@ -F '"snapshot_boundaries":"before-post-assoc-tail,m1-observation-timeout-5000ms"' "$identity" >/dev/null
