...
@grep@ -Fx 'PASSIVE_M1_TELEMETRY_CONTRACT=linux-6.18.40-passive-m1-rx-v2' "$manifest" >/dev/null
@grep@ -Fx 'SAFE_READ_REGISTERS=0xd4208,0xd4528,0xd452c' "$manifest" >/dev/null
@grep@ -Fx 'CONSUMING_MIB_READS=false' "$manifest" >/dev/null
@grep@ -Fx 'SNAPSHOT_BOUNDARIES=before-post-assoc-tail,first-data-timeout-1000ms' "$manifest" >/dev/null
...
@grep@ -Fx 'TELEMETRY_BEHAVIOR=best-effort-read-only-telemetry,control-flow-and-timeout-unchanged' "$manifest" >/dev/null
...
@grep@ -F '"artifact_identity":"mt7921-validation-v4"' "$identity" >/dev/null
...
@grep@ -F '"passive_m1_telemetry_contract":"linux-6.18.40-passive-m1-rx-v2"' "$identity" >/dev/null
@grep@ -F '"safe_read_registers":"0xd4208,0xd4528,0xd452c"' "$identity" >/dev/null
@grep@ -F '"consuming_mib_reads":false' "$identity" >/dev/null
@grep@ -F '"snapshot_boundaries":"before-post-assoc-tail,first-data-timeout-1000ms"' "$identity" >/dev/null
...
@grep@ -F '"behavior":"best-effort-read-only-telemetry,control-flow-and-timeout-unchanged"' "$identity" >/dev/null
@grep@ -F '"attribution_limit":"independent-ap-or-over-air-witness-required"' "$identity" >/dev/null
