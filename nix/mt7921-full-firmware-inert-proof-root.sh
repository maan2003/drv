...
@grep@ -Fx 'PASSIVE_M1_DIAGNOSTIC_CONTRACT=linux-6.18.40-safe-read-rx-dma-v1' "$manifest" >/dev/null
@grep@ -Fx 'NO_M1_RESULT=no_m1_at_rx_dma_ambiguous' "$manifest" >/dev/null
@grep@ -Fx 'CONSUMING_MIB_READS=false' "$manifest" >/dev/null
@grep@ -Fx 'DEFINITIVE_ATTRIBUTION_REQUIRES=independent-ap-or-over-air-witness' "$manifest" >/dev/null
...
@grep@ -F '"passive_m1_diagnostic_contract":"linux-6.18.40-safe-read-rx-dma-v1"' "$identity" >/dev/null
@grep@ -F '"no_m1_result":"no_m1_at_rx_dma_ambiguous"' "$identity" >/dev/null
@grep@ -F '"consuming_mib_reads":false' "$identity" >/dev/null
@grep@ -F '"definitive_attribution_requires":"independent-ap-or-over-air-witness"' "$identity" >/dev/null
