...
@grep@ -F '"passive_m1_telemetry_contract":"linux-6.18.40-passive-m1-rx-v3"' "$identity" >/dev/null
@grep@ -F '"safe_read_registers":"0xd4208,0xd4528,0xd452c"' "$identity" >/dev/null
@grep@ -F '"consuming_mib_reads":false' "$identity" >/dev/null
@grep@ -F '"snapshot_boundaries":"before-post-assoc-tail,first-data-timeout-5000ms"' "$identity" >/dev/null
...
@grep@ -F '"behavior":"best-effort-read-only-telemetry,control-flow-unchanged-except-observation-deadline-5000ms"' "$identity" >/dev/null
