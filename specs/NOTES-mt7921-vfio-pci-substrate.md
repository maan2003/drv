...
| Reset and teardown | Masks MAC/host sources, disables and polls WFDMA, clears BME, disables the VFIO IRQ, and releases resources. Some current operations, including `PrepareOwnedGlobalTxRings`, reset while DMA remains pinned and then read the old BAR mappings; the current active cleanup instead releases mappings before reset. | `reset` first invalidates the generation, retries IRQ revocation, revokes PCI DMA mappings, issues VFIO reset, and makes all issued BAR/DMA/IRQ handles stale. Failed DMA release is quarantined rather than forgotten. | The first cut-2 decision is whether to convert every operation to unmap-before-reset or add an explicit reset-while-pinned backend operation. Use pinned mt76's C teardown order—unregister/disable NAPI, take driver ownership, clean DMA, then reset WFSYS—as the reference. Normal LinuxVfio reset requires reopening BAR0/slices in the new generation for post-reset verification. If BME-off readback fails, retain the whole `Device` for watchdog recovery; do not release mappings or call reset. |
...
1. **Decide reset containment first.** Against pinned mt76's C teardown order,
   either convert every retained operation to unmap-before-reset or define the
   explicit reset-while-pinned backend contract. Do this before migrating BAR
   or DMA owners; normal reset invalidates them and post-reset checks must
   reopen BAR0 in the new generation.
