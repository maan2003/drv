        ...
        rec {
          ...
          mt7921-full-firmware-validation =
            ...
            postFixup = ''
              ...
              TELEMETRY_BEHAVIOR=best-effort-read-only-telemetry,control-flow-and-timeout-unchanged
            ...
            installCheckPhase = ''
              ...
              grep -F '"passive_m1_telemetry_contract":"linux-6.18.40-passive-m1-rx-v2"' $out/share/mt7921-full-firmware-validation/artifact-identity.json
              ...
              grep -F '"snapshot_boundaries":"before-post-assoc-tail,first-data-timeout-1000ms"' $out/share/mt7921-full-firmware-validation/artifact-identity.json
              ...
              grep -F '"behavior":"best-effort-read-only-telemetry,control-flow-and-timeout-unchanged"' $out/share/mt7921-full-firmware-validation/artifact-identity.json
              ...
              printf '%s\n' "$production_output" \
                | grep -F '"passive_m1_telemetry_contract":"linux-6.18.40-passive-m1-rx-v2"' \
                | grep -F '"safe_read_registers":"0xd4208,0xd4528,0xd452c"' \
                | grep -F '"consuming_mib_reads":false' \
                | grep -F '"snapshot_boundaries":"before-post-assoc-tail,first-data-timeout-1000ms"' \
                ...
                | grep -F '"behavior":"best-effort-read-only-telemetry,control-flow-and-timeout-unchanged"' \
          ...
          mt7921-full-firmware-validation-launcher-test =
            pkgs.runCommand "mt7921-full-firmware-validation-launcher-test"
              ...
              ''
                ...
                printf '%s\n' '{"artifact_identity":"mt7921-validation-v4","flavor":"full-firmware-production","enabled_operation":"run-one-shot-sae-auth","passive_m1_telemetry_contract":"linux-6.18.40-passive-m1-rx-v2","safe_read_registers":"0xd4208,0xd4528,0xd452c","consuming_mib_reads":false,"snapshot_boundaries":"before-post-assoc-tail,first-data-timeout-1000ms","positive_result":"target_m1_observed_at_rx_dma","negative_result":"no_m1_at_rx_dma_ambiguous","target_scope":"pinned-ap-to-client-exact-addr1-addr2-addr3-direction-and-eapol-key-m1","behavior":"best-effort-read-only-telemetry,control-flow-and-timeout-unchanged","attribution_limit":"independent-ap-or-over-air-witness-required","source_commit":"launcher-test","fd_contract":"credential-fd3+snapshot-fd4+immediate-eof","active_capable":true}' > work/var/artifact-identity.json
          ...
          mt7921-full-firmware-validation-supervisor = pkgs.runCommand
            ...
            ''
              ...
              grep -F '"passive_m1_telemetry_contract":"linux-6.18.40-passive-m1-rx-v2"' "$identity"
              grep -F '"safe_read_registers":"0xd4208,0xd4528,0xd452c"' "$identity"
              grep -F '"consuming_mib_reads":false' "$identity"
              grep -F '"snapshot_boundaries":"before-post-assoc-tail,first-data-timeout-1000ms"' "$identity"
              ...
              grep -F '"behavior":"best-effort-read-only-telemetry,control-flow-and-timeout-unchanged"' "$identity"
