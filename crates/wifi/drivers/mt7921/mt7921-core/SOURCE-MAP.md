...
| Post-association BSS power state | `mt7921/main.c::mt7921_bss_info_changed` (`BSS_CHANGED_PS`), `mt7921/mcu.c::mt7921_mcu_uni_bss_ps` (`UNI_BSS_INFO_PS`, tag 21) | `encode_client_post_assoc_power_state_command` | exact format, tested | mac80211 always publishes the vif PS state after association; the host has no power-save policy so it pins `ps_state` 0 (device awake), the `power_save off` form, right after BCNFT and before the beacon RX filter. |
