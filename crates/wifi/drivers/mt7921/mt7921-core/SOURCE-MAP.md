...
| Post-association BSS power state | `mt7921/main.c::mt7921_bss_info_changed` (`BSS_CHANGED_PS`), `mt7921/mcu.c::mt7921_mcu_uni_bss_ps` (`UNI_BSS_INFO_PS`, tag 21); `mt792x_core.c::mt792x_tx`; `mt76_connac.h::mt76_connac_pm_ref`; `mt792x_mac.c::mt792x_pm_wake_work` | `encode_client_post_assoc_power_state_command` | awake format exact; dynamic policy unsupported | Association and explicit Performance pin `ps_state` 0 (device awake). State 2 alone is not exposed: pinned Linux also gates every TX on its tracked PM ownership state and queues behind `mt792x_mcu_drv_pmctrl` before publication when firmware owns the HIF. Native does not yet own that wake boundary, and physical evidence showed acknowledged state 2 could strand post-idle TX. Balanced therefore returns unsupported rather than pretending awake is a power-saving mode. |
