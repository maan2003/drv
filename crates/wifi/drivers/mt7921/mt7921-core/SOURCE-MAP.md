...
| Post-association BSS power state | `mt7921/main.c::mt7921_bss_info_changed` (`BSS_CHANGED_PS`), `mt7921/mcu.c::mt7921_mcu_uni_bss_ps` (`UNI_BSS_INFO_PS`, tag 21); `mt792x_core.c::mt792x_tx`; `mt76_connac.h::mt76_connac_pm_ref`; `mt792x_mac.c::mt792x_pm_wake_work`; `mt792x_core.c::{__mt792xe_mcu_drv_pmctrl,mt792xe_mcu_fw_pmctrl}`; `mt792x_dma.c::mt792x_wpdma_reinit_cond` | `encode_client_post_assoc_power_state_command`, `DriverOwnershipWake`, `FirmwareOwnershipSleep`, production HIF gate and runtime WPDMA reinit | source-shaped | Balanced sends state 2 while driver-owned, arms MCU wake IRQ, verifies SET_OWN, and gates the complete operational MMIO body. IRQ or queued work drives bounded CLR_OWN; a cleared NEED_REINIT bit rebuilds descriptors/rings, revokes old RX authority, rebases software cursors while preserving MCU sequence, and restores exact IRQ masks before publication resumes. Performance requested from Balanced wakes before sending state 0. |
