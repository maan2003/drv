...
| `mt7921_mcu_fw_log_2_host`, post-CLC order in `mt7921_run_firmware` | `mt7921/mcu.c`, `mt76_connac_mcu.h` | `DownloadCommand::FirmwareLogToHost`, `run_firmware_loader` | exact, tested | 68-byte CE CID `0x400c5`, payload `01 00 00 00`, no-response TX completion, ordered after CLC and before channel-domain/passive setup |
...
| Associated data TX header translation | `mt792x_core.c` (`SUPPORTS_TX_ENCAP_OFFLOAD`), `mt76_connac_mac.c::mt76_connac2_mac_write_txwi_8023`, `net/mac80211/tx.c::ieee80211_8023_xmit` | `client_data_mpdu_to_ethernet`, `encode_client_data_txwi` (normal path) | exact format subset, tested | Linux hands mt7921 802.3 frames for associated data; the TXD says HDR_FORMAT 802.3/ETH_802_3/TID and the firmware builds the 802.11, QoS, sequence and CCMP headers from the WTBL (no `IEEE80211_KEY_FLAG_GENERATE_IV` for mt7921). The Fuchsia MLME emits 802.11 MPDUs, so the host reverses RFC 1042 (DA = addr3, SA = addr2) before DMA. Submitting the raw MPDU under that TXD produced unacknowledged garbage on air (TX_FREE status 1, 15 attempts) on 2026-09-07; after translation the AP ACKs within 1-5 attempts. |
...
| Operating channel switch reason | `mt7921/main.c::{mt7921_set_channel,mt7921_config}`, `mt7921/mcu.c::mt7921_mcu_set_chan_info` (`switch_reason`), `mt76_connac_mcu.h` (`CH_SWITCH_NORMAL`, `CH_SWITCH_SCAN_BYPASS_DPD`) | `ChannelSwitchReason`, `PassiveMcuCommand::ChannelSwitch::switch_reason`, `PhysicalChannelContext::switch_reason`, `Mt7921PassiveTransport::establish_client_channel` | exact field, tested | Linux switches the configured (operating) chandef with `CH_SWITCH_NORMAL`; only off-channel scanning uses `CH_SWITCH_SCAN_BYPASS_DPD`. The host used 9 for every switch, including the 80 MHz association chandef, and the radio then decoded only 6 Mbit/s frames (2026-09-07 runs 104739Z/110031Z: 0 of 76 Offers at 48 Mbit/s, our TX needing 3-5 attempts at -52 dBm). |
| Post-association BSS power state | `mt7921/main.c::mt7921_bss_info_changed` (`BSS_CHANGED_PS`), `mt7921/mcu.c::mt7921_mcu_uni_bss_ps` (`UNI_BSS_INFO_PS`, tag 21); `mt792x_core.c::mt792x_tx`; `mt76_connac.h::mt76_connac_pm_ref`; `mt792x_mac.c::mt792x_pm_wake_work`; `mt792x_core.c::__mt792xe_mcu_drv_pmctrl`; `mt792x_dma.c::mt792x_wpdma_reinit_cond` | `encode_client_post_assoc_power_state_command`, `DriverOwnershipWake` | awake format exact; nonblocking ownership mechanics tested; dynamic policy unsupported | Association and explicit Performance pin `ps_state` 0 (device awake). The deferred wake primitive preserves the caller epoch/deadline and Linux's bounded CLR_OWN retry shape without blocking the hardware actor. State 2 remains unexposed until every operational MMIO path is gated and post-wake WPDMA reinit can reset/rebase all DMA rings only after pending publication ownership is resolved. Physical evidence showed acknowledged state 2 could strand post-idle TX. Balanced therefore returns unsupported rather than pretending awake is a power-saving mode. |
...
CARGO_TARGET_DIR=/tmp/mt7921-full-baseline cargo test --locked \
  --manifest-path "$source/crates/wifi/drivers/mt7921/mt7921-passive-scan/Cargo.toml" \
