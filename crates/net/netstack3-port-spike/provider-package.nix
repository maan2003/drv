...
let
  ...
  source = runCommand "netstack3-provider-source-${commit}" {
    ...
    cp -R ${../../platform/directory-capability} "$refroot/src/lib/directory-capability"
    ...
    [workspace]
    members = ["crates/net/netstack3-port-spike"]
    resolver = "3"
    ...
    version = "0.1.0"
    CARGO
    mkdir -p "$out/crates/net/netstack3-port-spike"
    cp ${./Cargo.toml} "$out/crates/net/netstack3-port-spike/Cargo.toml"
    cp -R ${./src} "$out/crates/net/netstack3-port-spike/src"
    chmod -R u+w "$out"
