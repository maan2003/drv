...
#[cfg(all(test, feature = "fuchsia-passive"))]
#[derive(Clone, Debug, Eq, PartialEq)]
struct FreshTargetIdentity {
    bssid: [u8; 6],
    ssid: Vec<u8>,
    primary: fidl_fuchsia_wlan_ieee80211::ChannelNumber,
    bandwidth: fidl_fuchsia_wlan_ieee80211::ChannelBandwidth,
    vht_secondary_80_channel: fidl_fuchsia_wlan_ieee80211::ChannelNumber,
    rsne: Option<Vec<u8>>,
    rsnxe: Option<Vec<u8>>,
    protection: fidl_fuchsia_wlan_sme::Protection,
}
...
struct SelectedTargetSnapshot {
    owner: NonZeroU64,
    generation: u64,
    selected_at_nanos: i64,
    identity: FreshTargetIdentity,
}

#[cfg(all(test, feature = "fuchsia-passive"))]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct FreshScanWindow {
    txn_id: u64,
...
struct CurrentPowerEvidence {
    ...
    vht_secondary_80_channel: fidl_fuchsia_wlan_ieee80211::ChannelNumber,
    alpha2: [u8; 2],
    max_reg_power_dbm: u8,
    effective_cap_half_dbm: i8,
}

/// Fresh same-run evidence for a selected ordinary Fuchsia target. This is an
/// offline binding, not frame-publication authority.
#[cfg(all(test, feature = "fuchsia-passive"))]
#[derive(Debug, Eq, PartialEq)]
struct FreshTargetBinding {
    owner: NonZeroU64,
    generation: u64,
    scan_txn_id: u64,
    expires_at_nanos: i64,
    identity: FreshTargetIdentity,
    power: CurrentPowerEvidence,
}

#[cfg(all(test, feature = "fuchsia-passive"))]
fn exact_last_ie(ies: &[u8], wanted: wlan_common::ie::IeType) -> Result<Option<Vec<u8>>, String> {
    let mut found = None;
    for (ie_type, body) in wlan_common::ie::IeSummaryIter::new(ies) {
        if ie_type == wanted {
            let start = body
                .start
                .checked_sub(2)
                .ok_or_else(|| "information element omitted its header".to_string())?;
            found = Some(
                ies.get(start..body.end)
                    .ok_or_else(|| "information element range escaped scan result".to_string())?
                    .to_vec(),
            );
        }
    }
...
#[cfg(all(test, feature = "fuchsia-passive"))]
fn fresh_target_identity(
    ssid: &[u8],
    bss: &fidl_fuchsia_wlan_ieee80211::BssDescription,
) -> Result<FreshTargetIdentity, String> {
    let parsed = wlan_common::bss::BssDescription::try_from(bss.clone())
        .map_err(|error| format!("pinned BSS conversion rejected target: {error}"))?;
    if parsed.ssid.to_vec() != ssid {
        return Err("selected SSID disagrees with selected BSS description".into());
    }
    Ok(FreshTargetIdentity {
        bssid: bss.bssid,
        ssid: ssid.to_vec(),
        primary: bss.primary,
        bandwidth: bss.bandwidth,
        vht_secondary_80_channel: bss.vht_secondary_80_channel,
        rsne: exact_last_ie(&bss.ies, wlan_common::ie::IeType::RSNE)?,
        rsnxe: exact_last_ie(&bss.ies, wlan_common::ie::IeType::RSNXE)?,
        protection: parsed.protection().into(),
    })
}
...
impl ProvenanceSession {
    ...
    fn snapshot_selected_target(
        &self,
        selected: &fidl_fuchsia_wlan_sme::ConnectRequest,
        selected_at_nanos: i64,
    ) -> Result<SelectedTargetSnapshot, String> {
        if self.closed || self.poisoned {
            return Err("cannot snapshot a target from an invalidated run".into());
        }
        Ok(SelectedTargetSnapshot {
            owner: self.id,
            generation: self.generation,
            selected_at_nanos,
            identity: fresh_target_identity(&selected.ssid, &selected.bss_description)?,
        })
    }

    fn bind_fresh_selected_target(
        &self,
        selected: &SelectedTargetSnapshot,
        scan: FreshScanWindow,
        ordinary_final_bss: &fidl_fuchsia_wlan_ieee80211::BssDescription,
        power: CurrentPowerEvidence,
        now_nanos: i64,
    ) -> Result<FreshTargetBinding, String> {
        if self.closed
            || self.poisoned
            || selected.owner != self.id
            || selected.generation != self.generation
            || power.owner != self.id
            || power.generation != self.generation
        {
            return Err("fresh target evidence escaped its live run".into());
        }
        if selected.selected_at_nanos > scan.started_at_nanos
            || scan.started_at_nanos > scan.completed_at_nanos
            || scan.completed_at_nanos > power.captured_at_nanos
            || power.captured_at_nanos > now_nanos
            || now_nanos > scan.expires_at_nanos
        {
            return Err("fresh target evidence is stale or out of order".into());
        }
        if scan.primary != selected.identity.primary
            || power.primary != selected.identity.primary
            || power.bandwidth != selected.identity.bandwidth
            || power.vht_secondary_80_channel != selected.identity.vht_secondary_80_channel
            || power.firmware_generation == 0
            || power.channel_generation == 0
            || power.regulatory_generation == 0
            || power.max_reg_power_dbm > 20
            || i16::from(power.effective_cap_half_dbm) > i16::from(power.max_reg_power_dbm) * 2
        {
            return Err("fresh target channel or power evidence mismatched".into());
        }
        if !power
            .alpha2
            .iter()
            .all(|byte| byte.is_ascii_uppercase() || *byte == b'0')
        {
            return Err("fresh target regulatory country was invalid".into());
        }
        let final_identity = fresh_target_identity(&selected.identity.ssid, ordinary_final_bss)?;
        if final_identity != selected.identity {
            return Err("fresh ordinary Fuchsia scan no longer matched selected target".into());
        }

        let mut physical_matches = 0usize;
        for carried in self
            .carried_results
            .iter()
            .filter(|carried| carried.result.txn_id == scan.txn_id)
        {
            self.validate_produced(&carried.provenance)?;
            if carried.result.timestamp_nanos < scan.started_at_nanos
                || carried.result.timestamp_nanos > scan.completed_at_nanos
            {
                return Err("Produced observation timestamp escaped fresh scan".into());
            }
            if carried.result.bss.bssid != selected.identity.bssid {
                continue;
            }
            let produced_identity =
                fresh_target_identity(&selected.identity.ssid, &carried.result.bss)?;
            if produced_identity != selected.identity {
                return Err("fresh Produced observation disagreed with selected target".into());
            }
            physical_matches = physical_matches
                .checked_add(1)
                .ok_or_else(|| "fresh Produced match count exhausted".to_string())?;
        }
        if physical_matches == 0 {
            return Err("fresh scan had no matching Produced physical observation".into());
        }

        Ok(FreshTargetBinding {
            owner: self.id,
            generation: self.generation,
            scan_txn_id: scan.txn_id,
            expires_at_nanos: scan.expires_at_nanos,
...
mod tests {
    ...
    #[cfg(feature = "fuchsia-passive")]
    fn fresh_sae_scan_result(
        txn_id: u64,
        timestamp_nanos: i64,
    ) -> fidl_fuchsia_wlan_mlme::ScanResult {
        let mut result = b2a_scan_result(b'a');
        result.txn_id = txn_id;
        result.timestamp_nanos = timestamp_nanos;
        result.bss.capability_info |= 1 << 4;
        result.bss.ies = vec![
            0, 3, b'a', b'p', b'1', 1, 1, 0x82, 48, 20, 1, 0, 0, 15, 172, 4, 1, 0, 0, 15, 172, 4,
            1, 0, 0, 15, 172, 8, 128, 0,
        ];
        result
    }

    #[cfg(feature = "fuchsia-passive")]
    fn selected_connect_request(
        bss: fidl_fuchsia_wlan_ieee80211::BssDescription,
    ) -> fidl_fuchsia_wlan_sme::ConnectRequest {
        fidl_fuchsia_wlan_sme::ConnectRequest {
            ssid: b"ap1".to_vec(),
            bss_description: bss,
    ...
    fn retain_fresh_produced(
        ...
        assert!(session.inspect_next_result(|carried| assert_eq!(carried.result, *result)));
    }

    #[cfg(feature = "fuchsia-passive")]
    fn fresh_binding_fixture() -> (
        ProvenanceSession,
        SelectedTargetSnapshot,
        FreshScanWindow,
        fidl_fuchsia_wlan_ieee80211::BssDescription,
        CurrentPowerEvidence,
    ) {
        let mut session = ProvenanceSession::new();
        let result = fresh_sae_scan_result(91, 25);
        let selected = session
            .snapshot_selected_target(&selected_connect_request(result.bss.clone()), 10)
            .unwrap();
        retain_fresh_produced(&mut session, &result);
        let scan = FreshScanWindow {
            txn_id: 91,
            started_at_nanos: 20,
            completed_at_nanos: 30,
            expires_at_nanos: 50,
            primary: result.bss.primary,
        };
        let power = CurrentPowerEvidence {
            owner: session.id,
            generation: session.generation,
            firmware_generation: 3,
            channel_generation: 4,
            regulatory_generation: 5,
            captured_at_nanos: 40,
            primary: result.bss.primary,
            bandwidth: result.bss.bandwidth,
            vht_secondary_80_channel: result.bss.vht_secondary_80_channel,
            alpha2: *b"US",
            max_reg_power_dbm: 20,
            effective_cap_half_dbm: 20,
        };
        (session, selected, scan, result.bss, power)
    }

    #[cfg(feature = "fuchsia-passive")]
    #[test]
    fn ordinary_selected_target_binds_to_fresh_produced_result_and_current_power() {
        let (session, selected, scan, final_bss, power) = fresh_binding_fixture();
        let binding = session
            .bind_fresh_selected_target(&selected, scan, &final_bss, power, 45)
            .unwrap();
        assert_eq!(binding.owner, session.id);
        assert_eq!(binding.generation, session.generation);
        assert_eq!(binding.scan_txn_id, 91);
        assert_eq!(binding.identity, selected.identity);
        assert_eq!(binding.power, power);
        assert_eq!(binding.expires_at_nanos, 50);
    }

    #[cfg(feature = "fuchsia-passive")]
    #[test]
    fn fresh_target_binding_rejects_stale_mismatched_or_unpowered_evidence() {
        let (session, selected, scan, final_bss, power) = fresh_binding_fixture();
        assert!(
            session
                .bind_fresh_selected_target(&selected, scan, &final_bss, power, 51)
                .is_err()
        );

        let (session, selected, scan, final_bss, mut power) = fresh_binding_fixture();
        power.channel_generation = 0;
        assert!(
            session
                .bind_fresh_selected_target(&selected, scan, &final_bss, power, 45)
        ...
        let (session, selected, scan, final_bss, mut power) = fresh_binding_fixture();
        power.effective_cap_half_dbm = 41;
        assert!(
            session
                .bind_fresh_selected_target(&selected, scan, &final_bss, power, 45)
                .is_err()
        ...
        let (session, selected, mut scan, final_bss, power) = fresh_binding_fixture();
        scan.primary.number = 6;
        assert!(
            session
                .bind_fresh_selected_target(&selected, scan, &final_bss, power, 45)
                .is_err()
        ...
        let (session, selected, scan, mut final_bss, power) = fresh_binding_fixture();
        final_bss.ies[4] = b'2';
        assert!(
            session
                .bind_fresh_selected_target(&selected, scan, &final_bss, power, 45)
        ...
        let (mut session, selected, scan, final_bss, power) = fresh_binding_fixture();
        session.carried_results[0].result.bss.primary.number = 6;
        assert!(
            session
                .bind_fresh_selected_target(&selected, scan, &final_bss, power, 45)
                .is_err()
    ...
    #[cfg(feature = "fuchsia-passive")]
    #[test]
    fn selected_target_snapshot_rejects_ssid_disagreement() {
        let session = ProvenanceSession::new();
        let result = fresh_sae_scan_result(91, 25);
        let mut selected = selected_connect_request(result.bss);
        selected.ssid = b"other".to_vec();
        assert!(session.snapshot_selected_target(&selected, 10).is_err());
    }
