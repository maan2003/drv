...
| `__mt7921_mcu_set_clc` request/response format | `mt7921/mcu.c`, `pci_mcu.c`, `pci.c`, `mt792x_acpi_sar.c`, `mt792x_acpi_sar.h`, `mcu.h`, `mt76_connac_mcu.h` | `world_clc_commands`, `ClcSetCommand::expects_response`, `encode_clc_set_command`, `parse_clc_set_response`, loader transport | adapted, tested | installed-artifact rule fixture; `wait_resp` follows cap bit 0 and no-response success advances; TX ring 17/Q0 plus simultaneous WM ring 0 and WM2 ring 4 receive expectation; physical response-enabled path fail-closed |
