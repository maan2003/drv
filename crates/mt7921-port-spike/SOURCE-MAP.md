...
| Connac2 rate/SAR power initialization | `mt7921/main.c::mt7921_set_tx_sar_pwr`, `mt76_connac_mcu.c::{mt76_connac_mcu_set_rate_txpower,mt76_connac_mcu_rate_txpower_band,mt76_connac_mcu_build_sku}`, `mt76_connac_mcu.h` | `encode_conservative_rate_tx_power_commands`, `submit_conservative_rate_tx_power` | adapted conservative uniform-rate subset, tested; dormant/physical pending | Source channel lists/eight-channel batches/161-entry SKU, CE command, DMA-consumed boundary and mandatory post-batch PSE read; world regulatory and SAR bounds plus a project-owned cap are mandatory, while CLC remains separate opaque firmware policy |
