...
| `__mt7921_mcu_set_clc` request/response format | `mt7921/mcu.c`, `mt792x_acpi_sar.c`, `mt792x_acpi_sar.h`, `mcu.h`, `mt76_connac_mcu.h` | `world_clc_commands`, `encode_clc_set_command`, `parse_clc_set_response`, loader transport | adapted, tested | opaque `00` rule golden fixture including packed no-ACPI MTCL sentinel `0xff`; bounded response errors; explicit one-shot gate and reset containment |
