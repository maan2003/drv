...
| `wcn6750_ops` QCN9074 RX descriptor accessors | `hw.c:1103-1137` | `rx::Wcn6750RxDescriptor` | wcn6750-specific | Generated C differential over arbitrary complete 388-byte descriptors covers the selected MSDU/MPDU/attention accessors, MPDU-start tag, and address-2 validity/value; every truncation is rejected. |
...
| `ath11k_dp_peer_setup` / `ath11k_dp_peer_rx_pn_replay_config` | `dp.c:43-83`; `dp_rx.c:1153-1220` | `tx::ClientDataPath::{setup_peer,install_peer_key}` / `reo::PeerRxTids::install_peer_key` | ported-corrected | HTT peer-map admission precedes the 17 initial TID queues. Pairwise install updates every active queue with the pinned 48-bit WPA replay descriptor before publishing key index/security type; group replay remains host-owned. Partial publication quarantines the peer until reset, and completed identical retries do not reissue PN updates. |
