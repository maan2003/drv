...
impl QrtrSocket {
    ...
    /// Binds locally. Node and port zero request kernel-selected values.
    ...
    /// Registers a persistent lookup for the exact packed UAPI instance word.
    ...
    pub fn lookup(&self, service: u32, instance: u32, timeout: Duration) -> io::Result<QrtrAddr> {
        let deadline = deadline_after(timeout)?;
        self.subscribe(service, instance)?;
...
fn get_socket_addr(fd: RawFd, peer: bool) -> io::Result<QrtrAddr> {
    let mut raw = MaybeUninit::<SockAddrQrtr>::uninit();
    let mut length = addr_len();
    // SAFETY: raw is writable for length bytes; on success the kernel writes
    // the returned address and its initialized length.
    let result = unsafe {
        if peer {
            libc::getpeername(fd, raw.as_mut_ptr().cast(), &mut length)
        } else {
            libc::getsockname(fd, raw.as_mut_ptr().cast(), &mut length)
        }
    };
    cvt_zero(result)?;
    validate_addr_len(length)?;
    // SAFETY: success initialized the complete address, as established by the
    // checked returned structure length.
    unsafe { raw.assume_init() }.typed()
}

fn recv_from_fd(fd: RawFd, bytes: &mut [u8]) -> io::Result<(usize, QrtrAddr)> {
    let mut raw = MaybeUninit::<SockAddrQrtr>::uninit();
    let mut length = addr_len();
    // SAFETY: both buffers are writable for their supplied lengths and remain
    // live throughout recvfrom.
    let result = unsafe {
        libc::recvfrom(
            fd,
    ...
    validate_addr_len(length)?;
    // SAFETY: successful recvfrom initialized the complete source address, as
    // established by the checked returned structure length.
    let source = unsafe { raw.assume_init() }.typed()?;
    Ok((received, source))
}
