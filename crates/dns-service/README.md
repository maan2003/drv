...
```sh
cargo test --manifest-path crates/dns-service/Cargo.toml -- --test-threads=1
cargo build --release --manifest-path crates/dns-service/Cargo.toml --examples
```
...
```sh
bash crates/netstack3-port-spike/kernel-provider/production/run-tailscale-kvm.sh \
  "$KERNEL" "$ROOT" "$NEW_OUTPUT" crates/dns-service/lab/guest-init
```
