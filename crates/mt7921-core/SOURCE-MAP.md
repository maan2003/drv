...
| Associated BSS BASIC/QBSS context | `mt76_connac_mcu.c::{mt76_connac_mcu_uni_add_bss,mt76_connac_get_phy_mode,mt76_connac_get_phy_mode_v2}` | `encode_client_bss_command` | exact MT7921 infrastructure-client subset, tested | Active BSS index 0/OMAC 0/band 0/WMM 0, infrastructure-station connection type/state, BSSID, BMC and STA WCID 19, beacon/DTIM, local A+HT+VHT+HE PHY mode, local OFDM+HT+VHT+HE non-HT-basic PHY bitmap and QBSS state are emitted together before STA association. E2E80 corrects the prior `0x0001` placeholder to Linux's local 5-GHz `0x0078`. |
| Connac2 rate/SAR power initialization | `mt7921/main.c::{__mt7921_start,mt7921_set_tx_sar_pwr}`, `mt76_connac_mcu.c::{mt76_connac_mcu_set_rate_txpower,mt76_connac_mcu_rate_txpower_band,mt76_connac_mcu_build_sku}`, `mt76_connac_mcu.h` | `encode_conservative_rate_tx_power_commands`, `RateTxPowerAuthorizer`, `SourceExactPassiveMechanics::install_rate_tx_power` | adapted conservative uniform-rate subset, tested; temporary VFIO setup pending | Immediately after SET_RX_PATH, all eight no-response SET_RATE_TX_POWER pages are emitted contiguously with source channel lists/eight-channel batches/161-entry SKU. Userspace waits only for each DMA descriptor's consumption before safely reusing its single payload arena; it emits no intervening MCU fence or synthetic ACK. The SKU builder now exactly preserves 5-GHz CCK and all eight VHT-stride reserved entries at 127; native runtime body, raw-page and full-envelope SHA-256 goldens cover all 62 records. World regulatory and SAR bounds plus a project-owned cap are mandatory, while CLC remains separate opaque firmware policy. Reproducing native per-channel values remains blocked on one immutable, generation-bound snapshot containing configured TX ceiling, regulatory channel presence/disabled/max power, ordered end-exclusive frequency-ranged SAR, and optional structured per-rate overrides. The external cap must apply to semantic rates before serialization, never to the reserved 127 entries; the current scalar conservative policy intentionally does not invent missing inputs. |
