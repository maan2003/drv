...
| Connac2 PCI management/data TXWI/TXP and DMA publish shape | `net/mac80211/mlme.c::{ieee80211_nullfunc_get,ieee80211_send_nullfunc}`, `net/mac80211/tx.c::{ieee80211_tx_control_port,ieee80211_tx_h_check_control_port_protocol,ieee80211_tx_h_sequence}`, `mt7921/pci_mac.c::mt7921e_tx_prepare_skb`, `mt792x_core.c::mt792x_tx`, `mt7921/main.c`, `mt76_connac_mac.c::{mt76_connac2_mac_write_txwi,mt76_connac2_mac_write_txwi_80211,mt76_connac_write_hw_txp}`, `dma.c` | `encode_mt7921_5ghz_auth_tx`, `linux_qos_null_probe_reference`, `linux_qos_eapol_control_port_reference`, `run_mt7921_privacy_safe_mgmt_matrix` | exact format subsets, tested | Independent Linux v7.1 awake QoS-null and QoS control-port transcripts cover semantic tx_info flags, host MPDU sequence/TXD SN_VALID distinction, associated WCID 7 on normal VO qidx 3 rather than ALTX, fixed rate/retry/lifetime and every TXD/TXP/MPDU byte; the same-session physical discriminator publishes each reference once with unique token/PID and raw completion evidence. Management fixtures retain serialized correlation, mandatory wipe/reclaim and bounds. |
...
| Associated STA_REC/WTBL context and queue mapping | `mt7921/mcu.c::mt7921_mcu_sta_update`, `mt76_connac_mcu.c::{mt76_connac_mcu_sta_basic_tlv,mt76_connac_mcu_sta_tlv,mt76_connac_mcu_wtbl_generic_tlv,mt76_connac_mcu_sta_cmd}`, `mt76_connac.h::mt76_connac_lmac_mapping` | `encode_legacy_wme_add_wcid_command`, `linux_legacy_rate_context_reference` | exact legacy-WME subset, tested | BSS/OMAC/WCID/AID/STATE_ASSOC/PHY/RA and nested WTBL GENERIC/RX/HDR_TRANS/SMPS fields are source-decoded. E2E78 corrects the concrete live divergence where negotiated FIDL rates were ignored in favor of one hard-coded 6-Mbps rate. Linux reverse AC mapping independently confirms mac80211 VO 0 maps to LMAC qidx 3; `no_rx_trans=true` is RX decapsulation policy, not TX disable. HT/VHT/HE capability TLVs remain a separately visible omission. |
