...
| SME-managed SAE state/crypto/timers | `src/connectivity/wlan/lib/{sme/src/client/{protection,rsn,state},rsn/src/**,fcg-crypto/src/**}` | `SaeHandshake` is a narrow wrapper around the unchanged pinned `wlan-rsn::Supplicant`; it converts only pinned SAE frame/timer/auth-status updates and intentionally exposes no association, EAPOL, or key updates | Fuchsia BSD-2-Clause |
