...
| `fuchsia-softmac-port`: portable passive offload-scan hardware edge and state | `src/connectivity/wlan/lib/mlme/rust/src/{device.rs,client/scanner.rs}` plus the pinned SoftMAC/MLME/IEEE schema values | Extracts the scan/channel subset of `DeviceOps` behind a synchronous host trait and retains the pinned scanner's passive request validation, Time Unit conversion, scan identity, result, and completion behavior; the only adapter is a deterministic MT7921-shaped fake with no physical I/O path; upstream-derived host fixtures cover requests and beacon/completion events | Fuchsia BSD-2-Clause |
