...
| `zx-types`, `zx-status`, and host `zx` value facade | `sdk/rust/{zx-types,zx-status,zx}/src/**` | Exact pinned `zx-types` and `zx-status` sources are packaged unchanged. A path-shaped local `zx` facade re-exports their status/raw value API without exposing unavailable Zircon syscalls | Fuchsia BSD-2-Clause reference and value crates; local facade MIT OR Apache-2.0 |
