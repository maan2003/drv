...
| `fidl_fuchsia_wlan_ieee80211` host subset | `sdk/fidl/fuchsia.wlan.ieee80211/{channel,constants,fields,reason_code,rsn,status_code}.fidl` | Narrow schema binding replacement exports the pinned constants, MAC address/BSS description, channel/band/BSS/PHY/reason/status/key/cipher flexible enums (including named success, refusal, and SAE status values), channel number, and fixed-size HT/VHT capability and operation records required by WLAN common, FCG crypto, and WLAN driver values; exact known-value validation, explicit unknown-value construction, and record lengths are tested; no policy | Fuchsia BSD-2-Clause |
