...
| `fidl_fuchsia_wlan_mlme` host values | `sdk/fidl/fuchsia.wlan.mlme/{wlan_mlme,wlan_mlme_ext}.fidl` | Path-shaped schema binding exports the pinned scan, connect, authentication, association, key, AP start/stop, EAPOL, capability, stats, Minstrel, controlled-port, PMK, and SAE payload values consumed by SoftMAC MLME and SME; strict discriminants, nullable boxes, strict unions, bounds, and cross-schema dependencies are preserved and tested; MLME protocol transport remains excluded | Fuchsia BSD-2-Clause |
