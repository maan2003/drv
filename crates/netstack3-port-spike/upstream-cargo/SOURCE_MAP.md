...
| `fidl_fuchsia_wlan_common` host subset | `sdk/fidl/fuchsia.wlan.common/{driver_features,wlan_common}.fidl` | Narrow schema binding replacement exports only WLAN common's pinned TX-vector sentinel, flexible MAC/data-plane/implementation enums, and feature-support tables; exact discriminants, unknown-value round trips, and table defaults are tested; no transport or policy | Fuchsia BSD-2-Clause |
| `fidl_fuchsia_wlan_ieee80211` host subset | `sdk/fidl/fuchsia.wlan.ieee80211/{channel,constants,fields,reason_code,status_code}.fidl` | Narrow schema binding replacement exports the pinned constants, MAC address/BSS description, channel/band/BSS/PHY/reason/status flexible enums (including named success, refusal, and SAE status values), channel number, and fixed-size HT/VHT capability and operation records required by WLAN common/FCG crypto; exact known-value validation, explicit unknown-value construction, and record lengths are tested; no policy | Fuchsia BSD-2-Clause |
...
| `fidl_fuchsia_wlan_mlme` host subset | `sdk/fidl/fuchsia.wlan.mlme/wlan_mlme_ext.fidl` | Narrow schema binding replacement exports only the per-band capability and device-information records consumed by WLAN common capability derivation; boxed optional HT/VHT records and IEEE/common schema dependencies are preserved and tested; no MLME protocol transport or policy | Fuchsia BSD-2-Clause |
...
| `fidl_fuchsia_wlan_softmac` host subset | `sdk/fidl/fuchsia.wlan.softmac/softmac.fidl` | Narrow schema binding replacement exports only the flexible TX flag masks and TX metadata record required by WLAN common's rate-vector conversion; exact flag bits, unknown-bit retention, and record mapping are tested; no protocol transport or driver policy | Fuchsia BSD-2-Clause |
...
| `wlan-common`: IEEE 802.11 frame/IE parsing and writing, capability derivation, channel/rate-vector logic, BSS classification, SME compatibility, and security value conversion | `src/connectivity/wlan/lib/common/rust/src/**` | Cargo manifest plus `wlan-common-host.patch`: gates only the Zircon-status conversion and consumes the non-Fuchsia scan timestamp to keep byte-for-byte algorithms warning-clean; existing upstream VMO persistence and Fuchsia time paths remain target-gated; Cargo host unit tests are disabled to match pinned GN's `with_unit_tests` only on non-host builds | Fuchsia BSD-2-Clause |
