...
| `wlan-sme`: AP/client policy, scan scheduling, connect/roam/disconnect state machines, RSN integration, and MLME request/event handling | `src/connectivity/wlan/lib/sme/src/**` | Cargo manifest plus `wlan-sme-host.patch`: excludes only generated Fuchsia endpoint serving, corrects a generated-binding macro namespace and a host newtype comparison, while retaining pinned core policy; endpoint responders and scan-result VMO persistence explicitly report unavailable on host | Fuchsia BSD-2-Clause |
