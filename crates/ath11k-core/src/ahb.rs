...
impl<B: Backend> Wcn6750Interrupts<B> {
    ...
    pub fn split(self) -> (Wcn6750CeInterrupts<B>, Wcn6750DpInterrupts<B>) {
        (self.ce, self.dp)
    }
}

pub struct Wcn6750CeInterrupts<B: Backend> {
    routes: &'static [InterruptRoute],
    interrupts: Vec<Interrupt<B>>,
}

impl<B: Backend> Wcn6750CeInterrupts<B> {
...
impl<B: Backend> ath11k_ce::CeCompletionWait for Wcn6750CeInterrupts<B> {
...
pub fn dispatch_wcn6750_interrupt<B: Backend, R: ath11k_hal::Rings<B>>(
    irq: Wcn6750Irq,
    pipes: &mut ath11k_ce::CePipes<B>,
    mmio: &ath11k_platform_backend::MmioRegion<B>,
    remote_read_pointers: &mut ath11k_platform_backend::CoherentDma<
