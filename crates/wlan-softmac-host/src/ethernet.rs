...
pub struct BoundedNetstackProof {
    runner: EthernetRunner<DhcpService, HostEthernetDevice>,
...
impl HostEthernetDevice {
    ...
    /// # Safety
    ///
    /// `fd` must be one endpoint of the nonblocking Unix `SOCK_SEQPACKET`
    /// frame seam and must not be used by any other owner after this call.
