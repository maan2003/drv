...
use crate::tags::{
    ...
    WMI_TAG_ARRAY_UINT32, WMI_TAG_SERVICE_AVAILABLE_EVENT, WMI_TAG_SERVICE_READY_EVENT,
    WMI_TAG_SERVICE_READY_EXT_EVENT, WMI_TAG_SOC_HAL_REG_CAPABILITIES,
    WMI_TAG_SOC_MAC_PHY_HW_MODE_CAPS,
};
use crate::{Event, Transport, WmiError};

use super::{word, Decoder, EventDecoder, Ready, TlvIter};
...
pub struct ServiceReadyExt {
    /// The complete 76-byte-or-longer fixed value, preserving PPE fields.
        ...
        for tlv in TlvIter::new(event.tlvs()) {
            ...
            match tlv.tag {
                x if x == WMI_TAG_SERVICE_READY_EXT_EVENT.0 => {
                    ...
                    out.fixed = Some(tlv.value.to_vec());
                }
...
impl<T: Transport> EventStream<T> {
    ...
    pub fn wait_for_unified_ready(&mut self, deadline_ns: u64) -> Result<Ready, WmiError> {
        ...
        {
            return Decoder::<Ready>::new(WMI_READY_EVENTID).decode(self.pending.remove(index));
        }
        loop {
            let event = self
                .transport
            ...
            if event.id == WMI_READY_EVENTID {
                return Decoder::<Ready>::new(WMI_READY_EVENTID).decode(event);
            }
            self.pending.push(event);
        }
    }
