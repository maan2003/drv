...
@grep@ -F '"artifact_identity":"mt7921-validation-v10"' "$identity" >/dev/null
...
@grep@ -F '"passive_m1_telemetry_contract":"linux-6.18.40-passive-m1-rx-v11"' "$identity" >/dev/null
...
@grep@ -F '"safe_read_registers":"0xd4208,0xd4508,0xd450c,0xd4528,0xd452c,0xd4548,0xd454c,0x820d8108,0x820e5000,0x820e5004"' "$identity" >/dev/null
@grep@ -F '"consuming_mib_reads":false' "$identity" >/dev/null
@grep@ -F '"snapshot_boundaries":"before-associated-bss,after-associated-bss-rlm-before-sta-pump-15ms,after-post-association-tail,m1-observation-timeout-5000ms"' "$identity" >/dev/null
