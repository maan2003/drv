...
Architecture, requirement, and functional records describe the current project state and should normally agree with the code. User-requested gate records protect major governing constraints from agent-initiated change or reversal. During deliberate gradual change, an optional `## Status` section on a non-gate record can explain known current deviations. Linked Specs are not a planning system or general-purpose descriptive documentation. Do not use them as a historical archive, issue tracker, implementation plan, changelog, or catalog of implementation details.
...
A gate is an explicitly user-requested constraint that agents must not change or reverse on their own. Creating or performing any operation on a gate requires an explicit user request for that operation. `linked-specs-updating` defines the complete gate lifecycle and authorization rules.
...
- `GATE-*` is an explicitly user-requested barrier against agents changing or reversing a major governing constraint. It preserves the constraint and explains what the user is trying to accomplish. It does not describe how the constraint is implemented or managed.
...
If a user request contradicts a gate but does not explicitly request an operation on that gate, bring the conflict back to the user and leave the gate unchanged. Do not treat the conflicting request itself as implicit authorization.
...
Do not re-escalate an editorial or mechanical correction, or synchronization of a non-gate record to an exact semantic end state that the requester has already explicitly requested. Follow the separate gate authorization rules for every gate operation.
...
Treat non-gate records as current project knowledge, not unquestionable authority. If code and a non-gate record disagree, do not assume either one is authoritative. A gate remains governing until the user explicitly requests an authorized operation on it. When a conflict is substantive or the correct artifact is not immediately clear, escalate under the rule above rather than deciding independently.
