...
rsync -az --delete --exclude target --exclude .jj --exclude .git --exclude 'result*' --exclude reference \
  ...
  "$root/crates" "$root/lab" "$root/Cargo.toml" "$root/Cargo.lock" "$root/flake.nix" "$root/flake.lock" "$root/nix" "$root/AGENTS.md" \
...
if [ "${1:-}" = --test ]; then
  ssh -n -F "$cfg" "$target" "set -e; source /data/persist/src/drv-fast/.dev-env.sh >/dev/null 2>&1; \
    ...
    rsync -a --delete --exclude target /data/persist/src/drv/lab/ /data/persist/src/drv-fast/lab/; \
...
ssh -n -F "$cfg" "$target" 'cp /data/persist/src/drv/lab/mt7921/scripts/fast-build.sh /data/persist/drvlab/fast-build.sh; /data/persist/drvlab/fast-build.sh 2>&1 | tee /data/persist/drvlab/fast-build-last.log | grep -E "^[0-9:]+ \+|^error|^\s+-->" | tail -n 30'
