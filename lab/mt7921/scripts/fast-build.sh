...
patchsig=$( { cat "$REPO/nix/mt7921-fuchsia-source.nix" "$REPO/nix/fuchsia-reference.json"; \
             cat "$REPO"/crates/net/netstack3-port-spike/upstream-cargo/patches/*.patch; } | sha256sum | cut -d' ' -f1)
...
rsync -a --delete --exclude target "$REPO/lab/" "$WORK/lab/"
...
export MT7921_PROJECT_CORE_SOURCE_SHA256=$(cd "$WORK" && find crates/wifi/drivers/mt7921/mt7921-core -type f -print0 | sort -z \
...
cd "$WORK/crates/wifi/drivers/mt7921/mt7921-passive-scan"
step "cargo build"
