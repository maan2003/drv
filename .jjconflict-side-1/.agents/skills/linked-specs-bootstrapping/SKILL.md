...
Create a small initial set of records. Architecture, requirement, and functional records describe the current system:
...
- `REQ-*` and `SPEC-*` records only for knowledge that passes the strict qualification rules in `linked-specs-updating` and lacks a better existing source of truth
- no `GATE-*` records; do not establish gates while bootstrapping, and handle any gate request as a separate task under `linked-specs-updating`
